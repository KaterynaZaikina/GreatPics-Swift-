...
final public class InstaPostManager {
    ...
    //MARK: - Public methods
    func importPost(posts: AnyObject, completionBlock: (Void) -> (Void)) {
        let managedContext = CoreDataManager.sharedManager.managedObjectContext
        
        let fetchRequest = NSFetchRequest()
        let entity = NSEntityDescription.entityForName(Constants.entityName, inManagedObjectContext: managedContext)
        ...
        do {
            fetchedObjectArray = try managedContext.executeFetchRequest(fetchRequest) as? [InstaPost]
